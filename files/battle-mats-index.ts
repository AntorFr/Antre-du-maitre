...
export type BattleMatTag =
  | 'interieur' | 'exterieur'
  | 'foret' | 'donjon' | 'village' | 'montagne' | 'eau' | 'plaine' | 'souterrain'
  | 'batiment' | 'temple' | 'taverne' | 'marais' | 'desert' | 'lave' | 'glace' | 'cote'
  | 'combat' | 'exploration' | 'rencontre' | 'voyage';
...
export interface BattleMat {
  ...
  tags: BattleMatTag[];
  ambiances: string[];
  typesDefis: string[];       // ids de GAMEPLAY_TYPES compatibles
}

// Import des volumes (réexportés depuis leurs fichiers respectifs)
import { BATTLE_MATS_VOL1 } from './battle-mats-vol1';
import { BATTLE_MATS_VOL2 } from './battle-mats-vol2';
import { BATTLE_MATS_VOL3 } from './battle-mats-vol3';
